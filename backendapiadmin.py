...
def sendsms():
    if request.method == "POST":
        ...
        img.save(qrimgfilename+extension)
        
        qrcode_upload = Image.open(qrimgfilename+extension)
        coupon_upload = tempimage
        
        def get_concat_h(inputim1, inputim2):
            im1 = inputim1
            im2 = inputim2
            if im1.height > im2.height:
                fixed_height = im1.height
                height_percent = (fixed_height / float(im2.size[1]))
                width_size = int((float(im2.size[0])*float(height_percent)))
                im2 = im2.resize((width_size, fixed_height), PIL.Image.NEAREST)
                im2.save("testtestest.png")
            else:
                fixed_height = im2.height
                print(fixed_height)
                height_percent = (fixed_height / float(im1.size[1]))
                width_size = int((float(im1.size[0])*float(height_percent)))
                im1 = im1.resize((width_size, fixed_height), PIL.Image.NEAREST)
                im1.save("testtestest.png")
                
            #maxheight = max(im1.height, im2.height)
        ...
        def get_concat_v(inputim1, inputim2):
            im1 = inputim1
            im2 = inputim2
            if im1.width > im2.width:
                fixed_width = im1.width
                height_percent = (fixed_width / float(im2.width))
                height_size = int((float(im2.height)*float(height_percent)))
                im2 = im2.resize((fixed_width, height_size), PIL.Image.NEAREST)
                im2.save("testtestest.png")
            else:
                fixed_width = im2.width
                height_percent = (fixed_width / float(im1.width))
                height_size = int((float(im1.height)*float(height_percent)))
                im1 = im1.resize((fixed_width, height_size), PIL.Image.NEAREST)
                im1.save("testtestest.png")
                
            #maxwidth = max(im1.width, im2.width)
            dst = Image.new('RGB', (im1.width, im1.height + im2.height))
            dst.paste(im1, (0, 0))
            dst.paste(im2, (0, im1.height))
            return dst
        
        concat = get_concat_h(coupon_upload, qrcode_upload)
        #concat = get_concat_v(coupon_upload, qrcode_upload)
        

        fixed_width = 800
        width_percent = (fixed_width / float(concat.width))
        height_size = int((float(concat.height)*float(width_percent)))
        concat = concat.resize((fixed_width, height_size), PIL.Image.NEAREST)
        concatfilename = imgfilename + "_concat"
        concat.save(concatfilename+extension)

        
        """
        fixed_height = 800
        height_percent = (fixed_height / float(concat.height))
        width_size = int((float(concat.width)*float(height_percent)))
        concat = concat.resize((width_size, fixed_height), PIL.Image.NEAREST)
        concatfilename = imgfilename + "_concat"
        concat.save(concatfilename+extension)
        """
        
        #company = "스타벅스"
        #title = "맛있는 프라푸치노를 맛 보세요."
        #amount = "1000 TOP"
        company = ""
        try:
            cur.execute("SELECT company FROM companies WHERE userid = %s", (userid,))
        except:
            conn.rollback()
        else:
        ...
        for i in range(0, len(phone)):
            blankfilename = imgfilename + "_blank"
            img = np.zeros([int(concat.height/3.0),concat.width,3],dtype=np.uint8)
            img.fill(255) # numpy array!
            im = Image.fromarray(img)
            im.save(blankfilename + extension)
            im = Image.open(blankfilename + extension)
            draw = ImageDraw.Draw(im)
            font = ImageFont.truetype("NotoSansKR-Regular.otf", 26)
            deltaheight = im.height/3.0
            draw.text((60, 12),"기업     :  " + company,(0,0,0),font=font)
            draw.text((60, 12 + 28),"수신자 :  " + phone[i],(0,0,0),font=font)
            draw.text((60, 12 + 54),"쿠폰명 :  " + title,(0,0,0),font=font)
            im.save(blankfilename + phone[i] + extension)
        
            concat_upload = Image.open(concatfilename + extension)
            text_upload = Image.open(blankfilename + phone[i] + extension)
            
            concat2 = get_concat_v(concat_upload, text_upload)
            ...
            concat2filename = imgfilename + "_concat2"
            concat2.save(concat2filename + phone[i] + extension)
        ...
        for q in range(0, len(phone)):
            ...
            params["subject"] = "[터치콘] " + title[:40]

            params["image"] = concat2filename + phone[q] + extension
            ...
            if not sendtempbool:
                ...
                params['text'] = "[터치콘] "+ subject # Message
                #params["image"] = imgfilename+extension
                params["image"] = concatfilename + ".png"
        ...
        tupleval = (couponid, email, number, amount, imgfilename+extension, title, subject, currenttime(), True, True, json.dumps(thejsonb))
...
@app.route('/stakingadmin', methods=['GET', 'POST'])
@cross_origin(origin='*',headers=['Content- Type','Authorization'])
def stakingadmin():
    if request.method == "POST":
        ...
        key = "secret key"
        decoded = jwt.decode(token, key, algorithms="HS256")
        print(" ===========  decoded /stakingadmin     ", decoded)
        email = decoded["email"]

        print(" ===============   email   ", email)
        adminsList = []
        try:
            cur.execute("SELECT login FROM admins")
        except:
            adminsList = cur.fetchall()
        print(" ===============   adminsList12   ", adminsList)
        adminsList = [admin[0] for admin in adminsList]

        print(" ===============   adminsList   ", adminsList)

        if email == "touchcon" or email in adminsList:

            print(" ============= curr admin hit ")
            all_stakings = []
            try:
                cur.execute("SELECT id, date, userid, couponid FROM transactions WHERE type = %s", ("staking",))
            except:
                conn.rollback()
            else:
                all_stakings = cur.fetchall()
            if len(all_stakings) != 0 or all_stakings == None:
                
                resultvec = []
                for q in range(0, len(all_stakings)):
                    print(" =============    ", q)
                    account = all_stakings[q][3]
                    curr_date = all_stakings[q][1].split("_")
                    applicationdate = curr_date[0]
                    paymentdate = curr_date[1]
                    curr_userid = all_stakings[q][2]
                    user_email = ""
                    try:
                        cur.execute("SELECT email FROM users WHERE userid = %s", (curr_userid,))
                    except:
                        conn.rollback()
                    else:
                        user_email = cur.fetchall()[0][0]
                    tempdict = {
                        "Account": account,
                        "Email": user_email,
                        "ApplicationDate": applicationdate,
                        "PaymentDate": paymentdate,
                        "interest": "{:.0f}".format(float(account)*0.07)
                    }
                    print(" tempdict ", q, "   ", tempdict)
            ...
            return json.dumps({"Result": "관리자가 아닙니다."}), 200, {'ContentType':'application/json'}
